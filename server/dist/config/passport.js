    ...
    try {
        ...
        if (userRes.rows.length > 0) {
            ...
            if ((avatarUrl && userRes.rows[0].avatar_url !== avatarUrl) || (googleName && userRes.rows[0].google_name !== googleName)) {
                const updatedUser = await db.query('UPDATE users SET avatar_url = COALESCE($1, avatar_url), google_name = COALESCE($2, google_name) WHERE id = $3 RETURNING *', [avatarUrl, googleName, userRes.rows[0].id]);
        ...
        if (existingUser.rows.length > 0) {
            ...
            const updatedUser = await db.query('UPDATE users SET google_id = $1, avatar_url = COALESCE(avatar_url, $2), google_name = COALESCE(google_name, $3) WHERE id = $4 RETURNING *', [profile.id, avatarUrl, googleName, existingUser.rows[0].id]);
            return cb(null, updatedUser.rows[0]);
        }
        // Create new user
        const insertRes = await db.query('INSERT INTO users (username, google_id, avatar_url, google_name) VALUES ($1, $2, $3, $4) RETURNING *', [email, profile.id, avatarUrl, googleName]);
